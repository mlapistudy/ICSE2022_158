...
def solve_text_classify(outputs, find_keyword=False):
  ...
  for output in outputs:
    ...
    if name.endswith("__score"):
      if not value == find_default_value(fields, name):
        score[name.replace("__confidence","")] = value
  if len(labels)==0:
    if find_keyword:
...
# precondition
def solve_precondition(dest_file, target_function=None, API_inputs=[]):
  if not SOLVE_PRECONDITION:
    return API_inputs
  if len(API_inputs) == 0:
    return []
  ...
  src_file = options.test_file
  import change_code
  # change_code.change(src_file, dest_file, target_function)

  file_content = read_wholefile(dest_file)
  used_ml_api, ml_api_to_input, output_to_ml_api, If_statement_changes = get_ml_api(dest_file)
  if len(used_ml_api) == 0:
    return []
  
  input_vars = set()
  for item in ml_api_to_input:
    input_vars.add(item[1])
  if len(input_vars) > 1:
    print("[Error] Precondition only works for single input")
    return API_inputs
  input_var = input_vars.pop()
  # update input_var
  for line in file_content.split("\n"):
    if "# [Extra notation] API input from:" in line:
      tmp = line.replace("# [Extra notation] API input from:","").strip()
      tmp = eval(tmp)
      input_var = tmp[0]

  pre_condition = []
  for line in file_content.split("\n"):
    if "# [Extra notation] Function call of" in line:
      mentioned_api = line.strip().split()[-1]
      if mentioned_api in used_ml_api:
        pre_condition.append(line)
        break
    pre_condition.append(line)
  
  func_params = []
  file_content2 = read_wholefile(src_file)
  for line in file_content2.split("\n"):
    if line.strip().startswith("def"):
      func_name, func_params_, func_param_value = change_code.extract_function_info(line)
      if func_name == target_function:
        func_params = func_params_

  software_inputs = []
  for api_input in API_inputs:
    dest_file2 = src_file.replace(".py", "_partial.py")
    # input_var = "sentiment_text_call0_text" #test only
    with open(dest_file2, 'w', encoding='utf8') as file_obj:
      for line in pre_condition:
        file_obj.write(line+"\n")
      indent = change_code.extract_indent(pre_condition[-1])
      file_obj.write("%sif %s == \"%s\":\n" % (indent, input_var, str(api_input)))
      file_obj.write("%s  return \"%s\"\n" % (indent, MATCH_VALUE))

    try:
      software_input = []
      app = loaderFactory(dest_file2, target_function, refresh=True)
      engine = ExplorationEngine(app.createInvocation(), solver="cvc", print_info=False)
      generatedInputs, returnVals, path = engine.explore(40)
      for i in range(len(generatedInputs)):
        # print(str(generatedInputs[i]) + "\t-->\t" + str(returnVals[i]))
        if returnVals[i] == MATCH_VALUE:
          for value_pair in generatedInputs[i]:
            name, value = value_pair
            if name in func_params:
              software_input.append(value_pair)
          software_inputs.append(software_input)
          break
      if len(software_input) == 0: # fails to infer
        software_inputs.append(None)
...
if __name__ == '__main__':
  ...
  print("Testing API " + str(used_ml_api))
  ...
  try:
    ...
    for no, code in enumerate(base_code_file):
      ...
      if tmp.startswith("if ") or tmp.startswith("elif ") or tmp.startswith("for ") or tmp.startswith("while "):
        #TODO: if two statement are exactly the same
    ...
    def parse_statement_to_branch(Branches, covered_lines, base_code_file, test_code_file):
      ...
      for i, line_no in enumerate(covered_lines):
        if line_no in Branches.keys():
          ...
          jump_flag = False
          # TODO: manage 'else'. It won't appear in covered_lines
    ...
    #   print(solve_web(generatedInputs[i]))
    #   print(Solution_2_Path[i])
    #   print(Branches)
    ...
    for i in range(len(generatedInputs)):
      ...
      if len(used_ml_api) == 1:
        if used_ml_api[0] == "label_detection":
          ...
          Solution_2_ML_keywords.append([ssolve_text_classify(generatedInputs[i], find_keyword=True)])
          ...
          print("API reverse not supported: " + used_ml_api)
          sys.exit(1)
      else:
        used_vision_api = []
        used_language_api = []
        ...
        for api in used_ml_api:
          if api in Vision_API:
            ...
            print("API reverse not supported: " + api)
            sys.exit(1)
        ...
        if len(used_language_api)==0 and len(used_speech_api)==0:
          ...
          for api_set in group_ml_api(ml_api_to_input):
            if not "text_detection" in api_set and not "document_text_detection" in api_set:
              ...
              ML_keywords.append.append(("Vision APIs", "image of [" + str(search_keyword)+"]"))
              ML_keywords.append.append(("text_detection", "image with text [" + str(text)+"]"))
          ...
          print("API reverse not supported: " + str(used_ml_api))
          sys.exit(1)
      print("=======================\n\n")
      all_inputs.add('|\n|'.join(files))
    ...
    if options.func_params and len(used_ml_api)>1:
      print("--func-params currently is not supported for multiple API calls.")
      sys.exit(1)
    ...
    # ==================================================
    # preconditions - only for language api
    if set(used_ml_api).issubset(Language_API) and len(grouped_api) == 1:
      if options.func_params:
        refuse_set = values.keys()
      else:
        refuse_set = set()
      for item in all_inputs:
        generated_files = [x.strip() for x in item.split('|\n|')]
        revised = solve_precondition(filename, target_function=options.entry, API_inputs=generated_files)
        generated_files2 = generated_files.copy()
        for i in range(len(generated_files)):
          if revised[i] != None:
            for value_pair in revised[i]:
              name, value = value_pair
              if not name in refuse_set:
                generated_files2[i] = value
# python3.8 solve_precondition_full.py --m=25 --start=main -o test_suite/for_test/generated_test.txt -t test_suite/for_test/pre_on_language.py test_suite/for_test/pre_on_language_new.py
        ...
        for generated_files in item.split('|\n|'):
          ...
          try:
            if len(generated_files.split('|\t|')) == 1:
                    ...
                    if all_inputs_list[item_no] == item:
                      ...
                      for param in test_func_inputs:
                        if param == "API_input":
                          ...
                          command1 += " " + values[param][item_no]
                          exact_params.append(values[param][item_no])
              ...
              print("Do not support multiple files")
              sys.exit(1)
          except:
            pass
        f.write("\n=======================\n\n")
