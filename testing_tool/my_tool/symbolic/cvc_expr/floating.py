...
log = logging.getLogger("se.cvc.floating")

#newly added, modified from integer

class CVCFloat(CVCExpression):
    ...
    def variable(cls, name, solver):
        ...
        expr = em.mkVar(name, em.realType())
        return cls(expr, solver)
    
    @classmethod
    def constant(cls, v, solver):
        em = solver.getExprManager()
        num, deno = v.as_integer_ratio()
        return cls(em.mkConst(Rational(num, deno)), solver)
    ...
    def __add__(self, other):
        return CVCFloat(self.em.mkExpr(CVC4.PLUS, self.cvc_expr, other.cvc_expr), self.solver)

    def __sub__(self, other):
        return CVCFloat(self.em.mkExpr(CVC4.MINUS, self.cvc_expr, other.cvc_expr), self.solver)

    def __mul__(self, other):
        return CVCFloat(self.em.mkExpr(CVC4.MULT, self.cvc_expr, other.cvc_expr), self.solver)

    def __truediv__(self, other):
        return CVCFloat(self.em.mkExpr(CVC4.DIVISION, self.cvc_expr, other.cvc_expr), self.solver)

    # def __mod__(self, other):
    #     return CVCFloat(self.em.mkExpr(CVC4.INTS_MODULUS, self.cvc_expr, other.cvc_expr), self.solver)
