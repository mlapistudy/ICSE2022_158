...
class Loader:
	# Wan: I add an extra argument refresh to run multiple solvers in one script (e.g. solve_precondition). Not sure whether it would cause other error.
	def __init__(self, filename, entry, refresh=False):
		...
		self._resetCallback(True, refresh=refresh)
	...
	def _resetCallback(self,firstpass=False, refresh=False):
		self.app = None
		if firstpass and self._fileName in sys.modules and (not refresh):
...
def loaderFactory(filename,entry, refresh=False):
	...
	try: 
		...
		ret = Loader(filename,entry, refresh=refresh)
		return ret
	except ImportError:
		sys.path = sys.path[1:]
		return None
