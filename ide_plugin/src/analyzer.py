...
class CallGraphVisitor(ast.NodeVisitor):
    ...
    def visit_Attribute(self, node):
        ...
        if isinstance(node.ctx, ast.Store):
            ...
            self.logger.debug("attr_node is %s" % (attr_node))
            ...
            #
                ...
                self.logger.debug(
                    f"ns: {ns} tgt_name: {tgt_name} to_node: {to_node} defined: {to_node.defined}"
                )
                self.logger.debug(
                ...
                # STUB gather all namespace information about all ML APIs
                # to_node is <Node attribute:google.cloud.language_v1.LanguageServiceClient.analyze_sentiment>
                # if ns == "google.cloud.language_v1.LanguageServiceClient":
                #     if tgt_name == "analyze_sentiment":
                #         if self.add_uses_edge(from_node, to_node):
                #             self.logger.info("visit_Attribute: new uses edge from %s to %s" % (from_node, to_node))
    ...
    def visit_Call(self, node):
        # STUB[epic=modify] Probably add edges with ML API calls here?
    ...
    def add_uses_edge(self, from_node, to_node):
        ...
        if to_node.get_name() in All_API_Names:
        ...
        for n in self.uses_edges:
            for n2 in self.uses_edges[n]:
                if n2.namespace is not None and not n2.defined:
                    # STUB: gather all ML API namespace information
                    if n2.get_name() not in All_API_Names:
                    # ["google.cloud.language_v1.LanguageServiceClient.analyze_sentiment"]:
                    ...
                    # Else this is a ML API function call node
